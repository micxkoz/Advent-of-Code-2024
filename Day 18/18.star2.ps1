        ...
        foreach ($dir in $directions) {
            ...
            if ($next_x -ge 0 -and $next_x -lt $Width -and $next_y -ge 0 -and $next_y -lt $Height -and $Grid[$next_y][$next_x] -ne "#") {
                ...
                if ($cost_to_reach["$next_x,$next_y"] -eq -1 -or $next_cost -lt $cost_to_reach["$next_x,$next_y"]) {
                    ...
                    if (($queue | Where-Object {$_.x -eq $next_x -and $_.y -eq $next_y}).Count -eq 0) {
                        $queue += @{x=$next_x; y=$next_y}
                    }
                }
            }
        }
...
for ($i = $input_file.Length; $i -gt 0; $i--) {
    $memory_space = Get-GridFromFile $input_file $vertical_range $horizontal_range $i
    if ($(Get-ShortestPath $memory_space $vertical_range $horizontal_range) -ne -1) {return $input_file[$i]}
}
