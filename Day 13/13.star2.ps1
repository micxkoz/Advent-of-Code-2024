...
function Get-ParsedFile($File) {
    ...
    for ($line_counter = 0; $line_counter -lt $File.Length; $line_counter++) {
            ...
            if ($line_counter % 4 -eq 2) {
                ...
                $configuration += @($([long]($line_prepared[0] -replace "X=", "")+10000000000000), $([long]($line_prepared[1] -replace "Y=", "")+10000000000000))
...
function Get-ButtonPushes([long[]]$Configuration) {
    $c = $Configuration

    $button_b = (($c[5]*$c[0]) - ($c[1]*$c[4])) / ((-$c[2]*$c[1]) + ($c[3]*$c[0]))
    $button_a = ($c[4] - ($c[2]*$button_b)) / $c[0]

    if ($button_a -is [long] -and $button_b -is [long]) {return $button_a, $button_b}
    else {return $null}
}
...
foreach ($configuration in $all_machines_configurations) {
    ...
    if ($null -ne $a -and $null -ne $b) {$result += (3*$a) + $b}
}
$result